...
%set(fig_nondom,'units','normalized','outerposition',[0 0 1 1]); %we need to force full screen, otherwise the xtick that we set a few instructions below will be only 4, and then if I make the picture full-screen through the maximize-window button of Windows, the picture will have now 8 ticks, but those 4 ticks will be repeated for the other 4 ticks, finally generating wrong ticks for the x %
aux = xlim;
...
title('');
ylabel('Biomass [h^{-1}]');
xlabel('Total intracellular flux [mmolh^{-1}gDW^{-1}]');
