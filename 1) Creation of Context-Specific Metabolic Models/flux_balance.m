...
function [v, fmin, fmax] = flux_balance(fbamodel, quiet)
...
[v, fmin] = glpk(fbamodel.g, A, b, fbamodel.vmin, fbamodel.vmax, ctype, vartype, 1, param); %minimize total intracellular flux
fmin=norm(fbamodel.g.*v);
fmax=0;
if ~quiet
    fprintf('Biomass flux:    %f\n', fbamodel.f' * v)
    fprintf('Intracellular flux:  %f\n', norm(fbamodel.g.*v))
end